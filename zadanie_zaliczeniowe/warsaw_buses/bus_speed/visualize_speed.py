...
def show_speed_graph(speed_data):
    speed_data = speed_data.loc[speed_data["Speed"] > 10]
    avg_spd = speed_data.groupby("Lines")["Speed"].mean()
    avg_df = pandas.DataFrame({
        "Lines": speed_data["Lines"],
        "AvgSpeed": [avg_spd.to_dict()[line] for line in speed_data["Lines"]]
    }).drop_duplicates()
    smallest_and_largest = pandas.concat((avg_df.nsmallest(5, "AvgSpeed"),
                                          avg_df.nlargest(5, "AvgSpeed")))
    fig = px.bar(smallest_and_largest, x='Lines', y='AvgSpeed')
    fig.show()
